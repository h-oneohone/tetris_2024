...
class Tetromino:
    ...
    def create_pool(cls):
        ...
        if GAME_TYPE == "regular":
            cls.__POOL.append(
                Tetromino([[1, 0], [2, 0], [0, 1], [1, 1]], 4, -1, 1.0, 0.0, "S", 0, 4)
            )
            cls.__POOL.append(
                Tetromino([[0, 0], [1, 0], [1, 1], [2, 1]], 4, -1, 1.0, 1.0, "Z", 0, 4)
            )
            cls.__POOL.append(
            ...
            # cls.__POOL.append(
            #     Tetromino(
            #         [[0, 0], [0, 1], [1, 1], [2, 1], [3, 1]], 4, -1, 1.5, 0.5, "J", 0, 4
            #     )
            # )
            cls.__POOL.append(
            ...
            cls.__POOL.append(
                Tetromino([[1, 0], [1, 1], [2, 1], [2, 0]], 4, -1, 1.5, 0.5, "O", 0, 1)
            )
        # mini Tetris
        elif GAME_TYPE == "mini":
    ...
    def move(self, mov):
        ...
        if turn != 0:
            if (
                ...
                for i, sq in enumerate(self.tet):
                    # print(i)
                    sq[0] = change[self.type_str][self.rot][i][0]
                    sq[1] = change[self.type_str][self.rot][i][1]
                    # print(sq[0], sq[1])
            else:
                for sq in self.tet:
                    a = sq[0]
        ...
        #  print(self.center_x, self.center_y, self.rot_x, self.rot_y, self.rot, self.rot_max)
        return self
    ...
    def get_actions(self, state):
        ...
        if "hold" not in best_moves:
            if Tetromino_hung.type_str == "Z" or Tetromino_hung.type_str == "S":
                ...
                if count_turns_left == 3:
                    for i in range(count_turns_left):
                        best_moves.remove("turn left")
                    best_moves.insert(0, "turn right")
                if count_turns_left == 1:
                    if "right" in best_moves:
                        best_moves.remove("turn left")
                        best_moves.remove("right")
                        best_moves.insert(0, "turn right")

            if Tetromino_hung.type_str == "J" or Tetromino_hung.type_str == "L":
                if count_turns_left == 3:
                    for i in range(count_turns_left):
                        best_moves.remove("turn left")
                    best_moves.insert(0, "turn right")
        ...
        print("tetromino: ", self.env_hung.current_state.tetromino)
