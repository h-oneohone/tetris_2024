...
import time
import torch
import torch.nn as nn  # Import nn để sử dụng torch.nn.Module
...
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"  # Tắt GPU
...
class Game:
    ...
    def get_all_possible_states_conv2d_trong(self, state_trong=None):
        self.current_state = state_trong
        gamestates, moves, add_scores, dones, is_include_hold, is_new_hold = (
            self.get_all_possible_gamestates(state_trong)
        )
...
#         dir_path = os.path.dirname(os.path.realpath(__file__))
#         weight_file_path = os.path.join(dir_path, 'weight.pt')
#         self.model = keras.models.load_model(weight_file_path)
#         self.current_actions = []
#         self.ok = False
#         self.env_trong = Game(height=0)
...
#         Tetromino_trong = Tetromino.new_tetromino(piece_current_duy_convert[0])
#         piece_next_trongs = converted_piece
#         self.env_trong.current_state.define2(Tetromino_trong,piece_next_trongs,boolean_array)
#         states, add_scores, dones, _, _, moves = self.env_trong.get_all_possible_states_conv2d()
...
#         # print("Tetromino",Tetromino_trong.type_str)
#         if not ('hold' in best_moves):
#             if Tetromino_trong.type_str=='I' or Tetromino_trong.type_str=='O':
...
#                 self.env_trong.step(action=move)
#             elif move=='right':
...
#                 best_moves_convert.append(2)
#                 self.env_trong.step(action=move)
#         self.env_trong.step(chosen=best)
...
class ModelDQN(nn.Module):
    def __init__(self):
        ...
        self.max_pooling2d_1 = nn.MaxPool2d(
            kernel_size=(17, 7), stride=(1, 1), padding=0
        )
    ...
    def forward(self, batch):
        x = batch
        main_grid_input, hold_next_input = (
            x[0].reshape(-1, 1, 20, 10),
            x[1].reshape(-1, 45),
        )
        x_conv1 = F.relu(self.conv2d(main_grid_input))
        x_conv2 = F.relu(self.conv2d_1(main_grid_input))

        x1_max = self.max_pooling2d(x_conv1)
        x1_avg = self.average_pooling2d(x_conv1)
        x2_max = self.max_pooling2d_1(x_conv2)
        x2_avg = self.average_pooling2d_1(x_conv2)

        x1_max = x1_max.view(-1, 64)
        x1_avg = x1_avg.view(-1, 64)
        x2_max = x2_max.view(-1, 256)
        x2_avg = x2_avg.view(-1, 256)

        x0 = torch.cat((x1_max, x1_avg, x2_max, x2_avg, hold_next_input), dim=1)
...
#     def __init__(self):
#         super(ModelDQN, self).__init__()
#         self.conv1 = nn.Conv2d(1, 64, kernel_size=6)
#         self.conv2 = nn.Conv2d(1, 256, kernel_size=4)

#         self.max_pool1 = nn.MaxPool2d(kernel_size=(15, 5))
#         self.avg_pool1 = nn.AvgPool2d(kernel_size=(15, 5), stride=(15, 5))

#         self.max_pool2 = nn.MaxPool2d(kernel_size=(17, 7))
#         self.avg_pool2 = nn.AvgPool2d(kernel_size=(17, 7), stride=(17, 7))

#         self.flatten = nn.Flatten(start_dim=1)

#         self.__calculate_input_size()

#         self.dense1 = nn.Linear(self.fc_input_size, 128)
#         self.dense2 = nn.Linear(128, 64)
#         self.critic_output = nn.Linear(64, 1)

#     def __calculate_input_size(self):
#         x = torch.randn(1,1, 20, 10)

#         x1 = F.relu(self.conv1(x))
#         x2 = F.relu(self.conv2(x))

#         max_pool1 = self.max_pool1(x1)
#         avg_pool1 = self.avg_pool1(x1)
#         max_pool2 = self.max_pool2(x2)
#         avg_pool2 = self.avg_pool2(x2)

#         flat1 = self.flatten(max_pool1)
#         flat2 = self.flatten(avg_pool1)
#         flat3 = self.flatten(max_pool2)
#         flat4 = self.flatten(avg_pool2)
#         self.fc_input_size = flat1.size(1) + flat2.size(1) + flat3.size(1) + flat4.size(1) + 45

#     def forward(self, batch):

#         x = batch
#         main_grid_input, hold_next_input = x[0].reshape(-1, 1, 20, 10), x[1].reshape(-1, 45, 1)
...
#         flat3 = self.flatten(max_pool2)
#         flat4 = self.flatten(avg_pool2)
#         flat_hold = self.flatten(hold_next_input)

#         concat = torch.cat([flat1, flat2, flat3, flat4, flat_hold], dim=-1)

#         x = F.relu(self.dense1(concat))
...
class Agent:
    def __init__(self, turn):
        # Đường dẫn tới file trọng số PyTorch
        dir_path = os.path.dirname(os.path.realpath(__file__))
        weight_file_path = os.path.join(dir_path, "weight24000.pt")

        # Tải trọng số (state_dict)
    ...
    #     # Đường dẫn tới file trọng số PyTorch
    #     dir_path = os.path.dirname(os.path.realpath(__file__))
    #     model_file_path = os.path.join(dir_path, 'weight.pth')

    #     # # Tải trọng số (state_dict)
    #     # state_dict = torch.load(weight_file_path, map_location=torch.device('cpu'))

    #     self.model = torch.load(model_file_path, map_location=torch.device('cpu'))
    ...
    def format_data_infer(self, states):
        return (
            torch.from_numpy(states[0]).to(dtype=torch.float32),
            torch.from_numpy(states[1]).to(dtype=torch.float32),
        )
    ...
    def get_actions(self, state):
        ...
        new_row = np.array([0.0] * 10)
        for i in range(count):
            broad_2010 = np.insert(broad_2010, 0, new_row, axis=0)

        threshold = 1.0
        boolean_array = (broad_2010 == threshold).tolist()
        ...
        self.env_trong.current_state.define2(
            Tetromino_trong, piece_next_trongs, boolean_array
        )
        states, add_scores, dones, _, _, moves = (
            self.env_trong.get_all_possible_states_conv2d()
        )
        rewards = get_reward(add_scores, dones)

        self.model.eval()
        # Dự đoán Q-values bằng PyTorch
        with torch.no_grad():
            q = rewards + self.model(self.format_data_infer(states)).numpy()
        ...
        # Điều chỉnh theo loại Tetromino
        if "hold" not in best_moves:
            if Tetromino_trong.type_str in ["I", "O"]:
                if "right" in best_moves:
                    best_moves.remove("right")
                else:
                    drop_index = best_moves.index("drop")
            ...
            if Tetromino_trong.type_str in ["Z", "S"]:
                if count_turns_left > 0:
                    for _ in range(count_turns_left):
                        best_moves.remove("turn left")
                    best_moves.insert(0, "turn right")
            ...
            if Tetromino_trong.type_str == "J" and count_turns_left in [2, 3]:
                if "right" in best_moves:
                    best_moves.remove("right")
                else:
                    drop_index = best_moves.index("drop")
            ...
            if Tetromino_trong.type_str == "L" and count_turns_left in [1, 2]:
                if "left" in best_moves:
                    best_moves.remove("left")
                else:
                    drop_index = best_moves.index("drop")
        ...
        print("tu: best_moves after", best_moves)
        print("tetromino: ", self.env_trong.current_state.tetromino)
        for move in best_moves:
            action_map = {
                "left": 6,
                "right": 5,
            ...
            best_moves_convert.append(action_map.get(move, 2))
            self.env_trong.step(action=move)
